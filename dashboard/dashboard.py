...
with tab1:
    ...
    if st.button("Run Prediction"):
        ...
        prediction = predict_customer_spending(input_data=input_data, model_params=model_params)
    ...
    if st.session_state["prediction"] is not None:
        ...
        st.metric("Predicted Customer Spending", f"RM {st.session_state['prediction']:,.2f}")
        st.metric("Prediction Latency", f"{st.session_state['latency_ms']:.2f} ms")
...
As a marketing manager, I want the customer spending prediction model retrained using recent customer behaviour data so that spending forecasts remain reliable as customer behaviour changes.
    """)
